...
export default function AdminLayout() {
  ...
  const handlePublicSiteReturn = async () => {
    if (
      window.confirm(
        "SÉCURITÉ : Retourner sur le site public va détruire votre session Administrateur. Voulez-vous vous déconnecter et continuer ?",
      )
    ) {
      await signOut();
      navigate("");
          ...
          <div className="flex items-center gap-4">
            ...
            <Link to="/admin/dashboard" className="flex items-center gap-3">
              ...
              <div className="hidden md:block">
                <h1 className="text-lg tracking-tight text-[#0a0f1e] font-bold">
                  MSF CONGO
                </h1>
                <p className="text-[8px] text-[#d4af37] tracking-[0.2em] uppercase">
                  Admin Panel
                </p>
              </div>
            </Link>
      ...
      >
        ...
        <nav className="px-4 pb-4 space-y-2 flex-1 overflow-y-auto">
          {navigation.map((item) => {
            ...
            const isSuperadminOnly =
              "superadminOnly" in item ? Boolean(item.superadminOnly) : false;
        ...
        <div className="p-4 mt-auto border-t border-gray-100">
          <div className="bg-gray-50 rounded-xl border border-gray-200 p-4">
            <h3 className="text-sm text-[#0a0f1e] font-bold mb-3">
              Aujourd'hui
            </h3>
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-[#0a0f1e] font-black">
                  {unreadContactsCount}
                </span>
              </div>
              <div className="flex items-center justify-between">
