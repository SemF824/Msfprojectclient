...
export function FeaturedProperties({ filters }: FeaturedPropertiesProps) {
  ...
  const formatPrice = (price: string | number) => {
    if (typeof price === "number") return `${new Intl.NumberFormat("fr-FR").format(price)} FCFA`;
    return price;
  };
      ...
      <div className="container mx-auto">
        <div className="text-center mb-12 md:mb-16">
          <motion.div
            ...
            viewport={{ once: true }}
            className="inline-flex items-center gap-2 px-3 py-1.5 md:px-4 md:py-2 rounded-full bg-[#1e3a5f]/40 backdrop-blur-md border border-[#d4af37]/30 mb-4"
          >
            <TrendingUp className="w-3 h-3 md:w-4 md:h-4 text-[#d4af37]" />
            <span className="text-[10px] md:text-xs text-[#d4af37] tracking-wider uppercase">Collection Premium</span>
          </motion.div>
          <motion.h2
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <p className="text-gray-400 text-sm md:text-base">Chargement du catalogue...</p>
          </div>
        ) : filteredProperties.length === 0 ? (
          <div className="text-center text-sm md:text-base text-gray-400 py-12 bg-white/5 backdrop-blur-md rounded-2xl border border-white/10">
            ...
            {filteredProperties.map((property, index) => (
              ...
              >
                <div className="relative h-56 md:h-64 overflow-hidden">
                  ...
                  <div className="absolute top-3 left-3 md:top-4 md:left-4 px-2.5 py-1 bg-[#0a0f1e]/70 backdrop-blur-md text-white text-[10px] md:text-xs font-medium rounded-full border border-[#d4af37]/40">
                    {property.type}
                  </div>
                </div>
