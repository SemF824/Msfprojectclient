...
// Hack anti-clignotement Safari — appliqué UNIQUEMENT sur le wrapper d'animation,
// jamais sur l'élément portant backdrop-blur (sinon ça empire les choses).
...
export function FeaturedProperties({ filters }: FeaturedPropertiesProps) {
  ...
  const filteredProperties = properties.filter((property) => {
    ...
    if (filters.type !== "all") {
      ...
      const expectedType = typeMap[filters.type] || filters.type;
      ...
      <div className="container mx-auto">
        ...
        <div className="text-center mb-12 md:mb-16">
          <motion.div
            ...
            viewport={{ once: true }}
            style={safariFix}
            className="inline-flex items-center gap-2 px-3 py-1.5 md:px-4 md:py-2 rounded-full bg-[#1e3a5f]/60 md:bg-[#1e3a5f]/40 md:backdrop-blur-md border border-[#d4af37]/30 mb-4"
          >
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {filteredProperties.map((property, index) => (
              /*
               * ─── CORRECTION ANTI-CLIGNOTEMENT SAFARI ───────────────────────────
               *
               * AVANT : motion.div portait À LA FOIS l'animation ET le backdrop-blur.
               * Quand Safari anime opacity 0→1 sur un élément avec backdrop-filter,
               * il recalcule le filtre à chaque frame intermédiaire → flash/clignotement.
               *
               * APRÈS : deux couches séparées —
               *   1. motion.div  → gère UNIQUEMENT l'animation (opacity + y).
               *                    Pas de backdrop-blur, pas de border-radius overflow.
               *   2. div interne → porte le style visuel (backdrop-blur, border,
               *                    overflow:hidden, hover effects).
               *                    Cet élément n'est JAMAIS animé directement.
               *
               * Safari peut ainsi composer le backdrop-filter une seule fois,
               * et laisser la couche d'animation faire son travail sans interférence.
               * ───────────────────────────────────────────────────────────────────
               */
              <motion.div
                key={property.id}
                ...
                style={safariFix}
                // Pas de will-change-transform ici : combiné à backdrop-blur
                // sur un enfant, ça force Safari à créer un stacking context
                // supplémentaire → flicker garanti.
                className="flex flex-col"
              >
                {/*
                 * Couche visuelle : backdrop-blur sur un élément STATIQUE.
                 * Sur mobile on remplace backdrop-blur-xl par un fond solide
                 * (économie GPU significative sur WebKit mobile).
                 */}
                <div className="group relative bg-[#0d1b2e]/90 md:bg-white/5 md:backdrop-blur-xl rounded-2xl overflow-hidden border border-white/10 hover:border-[#d4af37]/50 transition-all duration-500 hover:shadow-2xl hover:shadow-[#d4af37]/20 flex flex-col flex-grow">
