...
export default function Dashboard() {
  ...
  // Données dynamiques depuis Supabase avec fallbacks robustes
  ...
  // Statistiques calculées avec sécurité
  const stats = [
    ...
    const fetchAllData = async () => {
      ...
      try {
        ...
        const [
          ...
          supabase.from('transactions').select('*').eq('user_id', userId).order('created_at', { ascending: false }).catch(() => ({ data: [] })),
          supabase.from('appointments').select('*').eq('user_id', userId).order('appointment_date', { ascending: true }).catch(() => ({ data: [] })),
          supabase.from('documents').select('*').eq('user_id', userId).order('created_at', { ascending: false }).catch(() => ({ data: [] })),
          supabase.from('favorites').select('*').eq('user_id', userId).catch(() => ({ data: [] })),
          supabase.from('devis_requests').select('*').eq('client_email', userEmail).order('created_at', { ascending: false }).catch(() => ({ data: [] })),
          supabase.from('notifications').select('*').eq('user_id', userId).order('created_at', { ascending: false }).limit(10).catch(() => ({ data: [] }))
        ]);
  ...
  const calculateMonthlyPayment = () => {
    ...
    if (principal <= 0 || loanTerm <= 0 || interestRate < 0) return 0;
    if (interestRate === 0) {
  ...
  const getTransactionStatusColor = (status: string) => {
    switch (status) {
      ...
      case "echoue": return "bg-red-500/20 text-red-400 border-red-500/30";
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white backdrop-blur-xl rounded-2xl border border-gray-200 shadow-lg p-6 sticky top-24">
              ...
              <div className="mb-4 pb-4 border-b border-gray-200">
                <Link to="/" className="flex items-center gap-2 px-4 py-2 text-sm text-gray-500 hover:text-[#d4af37] transition-colors rounded-xl hover:bg-gray-50">
                ...
                {[
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-colors ${
                      activeTab === item.id ? "bg-[#d4af37] text-[#0a0f1e]" : "text-gray-600 hover:bg-gray-100"
                    }`}
                  >
              ...
              <div className="pt-6 border-t border-gray-200 space-y-2">
                <Link to="/profile" className="w-full flex items-center gap-3 px-4 py-3 text-gray-600 hover:bg-gray-100 rounded-xl transition-colors">
                  <User className="w-5 h-5" /><span>Mon Profil</span>
                </Link>
                <Link to="/favorites" className="w-full flex items-center gap-3 px-4 py-3 text-gray-600 hover:bg-gray-100 rounded-xl transition-colors">
                  <Heart className="w-5 h-5" /><span>Mes Favoris</span>
                </Link>
                <Link to="/notifications" className="w-full flex items-center gap-3 px-4 py-3 text-gray-600 hover:bg-gray-100 rounded-xl transition-colors">
                  <Bell className="w-5 h-5" /><span>Notifications</span>
                </Link>
                <Link to="/settings" className="w-full flex items-center gap-3 px-4 py-3 text-gray-600 hover:bg-gray-100 rounded-xl transition-colors">
          ...
          <div className="lg:col-span-3">
            <div className="flex items-center justify-between mb-8">
              ...
              <Link to="/notifications" className="relative p-3 bg-white border border-gray-200 rounded-xl text-gray-600 hover:border-[#d4af37] hover:text-[#d4af37] transition-colors shadow-sm group">
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  {(stats || []).map((stat, idx) => {
                    ...
                    return (
                      <motion.div key={idx} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} className="bg-white backdrop-blur-xl rounded-2xl border border-gray-200 shadow-lg p-6">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white backdrop-blur-xl rounded-2xl border border-gray-200 shadow-lg p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-xl text-[#0a0f1e] font-semibold">Transactions Récentes</h2>
                    <Link to="/transactions" className="text-sm text-[#d4af37] hover:underline flex items-center gap-1">
                      <CreditCard className="w-4 h-4" /><span>Voir Tout</span>
                    </Link>
                  </div>
                  {isDataLoading ? (
                    <div className="flex justify-center py-8"><div className="w-8 h-8 border-4 border-[#d4af37] border-t-transparent rounded-full animate-spin"></div></div>
                    ...
                    <div className="space-y-3">
                      {(transactions || []).slice(0, 3).map((transaction) => (
                        <div key={transaction?.id || Math.random()} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200">
                          <div className="flex-1">
                            <p className="text-[#0a0f1e] font-medium">{transaction?.property_name || "Propriété inconnue"}</p>
                            <p className="text-sm text-gray-600">{(transaction?.amount || 0).toLocaleString()} FCFA</p>
                          </div>
                          <span className={`px-3 py-1 text-xs rounded-full border ${getTransactionStatusColor(transaction?.status || "inconnu")}`}>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white backdrop-blur-xl rounded-2xl border border-gray-200 shadow-lg p-6">
                  <h2 className="text-xl text-[#0a0f1e] font-semibold mb-4">Prochains Rendez-vous</h2>
                  {isDataLoading ? (
                    <div className="flex justify-center py-8"><div className="w-8 h-8 border-4 border-[#d4af37] border-t-transparent rounded-full animate-spin"></div></div>
                  ) : (appointments || []).length === 0 ? (
                    <EmptyState icon={Calendar} title="Aucun rendez-vous" description="Vous n'avez pas de rendez-vous planifié pour le moment." />
                  ) : (
                    <div className="space-y-3">
                      {(appointments || []).slice(0, 3).map((apt) => (
                        <div key={apt?.id || Math.random()} className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl border border-gray-200">
                          <div className="w-12 h-12 bg-[#d4af37]/20 rounded-xl flex items-center justify-center">
                            <Calendar className="w-6 h-6 text-[#d4af37]" />
                          </div>
                          <div className="flex-1">
                            <p className="text-[#0a0f1e] font-medium">{apt?.title || "Rendez-vous"}</p>
                          ...
                          <span className={`px-3 py-1 text-xs rounded-full border ${getAppointmentStatusColor(apt?.status || "inconnu")}`}>
            ...
            {activeTab === "requests" && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white backdrop-blur-xl rounded-2xl border border-gray-200 shadow-lg p-6">
                <h2 className="text-2xl text-[#0a0f1e] font-semibold mb-6">Mes Demandes de Devis</h2>
                {isDataLoading ? (
                  <div className="flex justify-center py-12"><div className="w-12 h-12 border-4 border-[#d4af37] border-t-transparent rounded-full animate-spin"></div></div>
                ) : (devisRequests || []).length === 0 ? (
                  <EmptyState icon={FileText} title="Aucune demande de devis" description="Vous n'avez pas encore soumis de demande de devis." />
                ) : (
                  <div className="space-y-4">
                    {(devisRequests || []).map((request) => (
                      <div key={request?.id || Math.random()} className="p-6 bg-gray-50 rounded-xl border border-gray-200">
                        <div className="flex items-start justify-between mb-4">
                          <div>
                            <h3 className="text-[#0a0f1e] text-lg font-medium mb-1">{request?.property_name || "Propriété"}</h3>
                            <p className="text-gray-600 text-sm">
                              Type: {request?.request_type === 'achat' ? 'Achat' : request?.request_type === 'location' ? 'Location' : 'Information'}
                            </p>
                          </div>
                          <span className={`px-3 py-1 text-xs rounded-full border ${getTransactionStatusColor(request?.status || "inconnu")}`}>
                            {getStatusLabel(request?.status || "")}
                          </span>
                        </div>
                        <div className="flex items-center justify-between pt-4 border-t border-gray-200">
                          <p className="text-gray-600 text-sm">
                            Soumise le {new Date(request?.created_at || new Date()).toLocaleDateString('fr-FR')}
                          </p>
                          <p className="text-[#d4af37] font-medium">
                            {(request?.property_price || 0).toLocaleString()} FCFA
                          </p>
                        </div>
                      </div>
                    ))}
            ...
            {activeTab === "appointments" && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white backdrop-blur-xl rounded-2xl border border-gray-200 shadow-lg p-6">
                ...
                {isDataLoading ? (
                  ...
                  <EmptyState icon={Calendar} title="Aucun rendez-vous" description="Vous n'avez pas encore de rendez-vous planifié." />
                ) : (
                  <div className="space-y-4">
                    {(appointments || []).map((apt) => {
                      ...
                      return (
                        <div key={apt?.id || Math.random()} className="p-6 bg-gray-50 rounded-xl border border-gray-200">
                          <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1">
                              <h3 className="text-[#0a0f1e] text-lg font-medium mb-1">{apt?.title || "Rendez-vous"}</h3>
            ...
            {activeTab === "favorites" && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white backdrop-blur-xl rounded-2xl border border-gray-200 shadow-lg p-6">
                ...
                {isDataLoading ? (
                  ...
                  <EmptyState icon={Heart} title="Aucune propriété favorite" description="Vous n'avez pas encore ajouté de propriété à vos favoris." />
                ) : (
                  <div className="space-y-4">
                    {(favorites || []).map((fav) => (
                      <div key={fav?.id || Math.random()} className="p-6 bg-gray-50 rounded-xl border border-gray-200">
                        <div className="flex items-center justify-between">
                          <div>
                            <h3 className="text-[#0a0f1e] text-lg font-medium mb-1">
                              Propriété ID: {fav?.property_id || "Inconnu"}
                            </h3>
                            <p className="text-gray-600 text-sm">
                              Ajoutée le {new Date(fav?.created_at || new Date()).toLocaleDateString('fr-FR')}
                            </p>
                          </div>
            ...
            {activeTab === "history" && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white backdrop-blur-xl rounded-2xl border border-gray-200 shadow-lg p-6">
                ...
                {isDataLoading ? (
                  ...
                  <EmptyState icon={Clock} title="Aucun historique" description="Vous n'avez pas encore de transaction terminée." />
                ) : (
                  <div className="space-y-4">
                    {(transactions || []).filter(t => t?.status === 'complete').map((transaction) => (
                      <div key={transaction?.id || Math.random()} className="p-6 bg-gray-50 rounded-xl border border-gray-200">
                        <div className="flex items-start justify-between mb-3">
                          <div>
                            <h3 className="text-[#0a0f1e] text-lg font-medium">{transaction?.property_name || "Propriété"}</h3>
                            <p className="text-gray-600 text-sm">
                         ...
                         <p className="text-gray-700 text-sm">
                           Type: {transaction?.transaction_type === 'acompte' ? 'Acompte' : transaction?.transaction_type === 'mensualite' ? 'Mensualité' : transaction?.transaction_type === 'frais' ? 'Frais' : (transaction?.transaction_type || "Autre")}
                         </p>
                      </div>
                    ))}
                  </div>
            ...
            {activeTab === "documents" && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white backdrop-blur-xl rounded-2xl border border-gray-200 shadow-lg p-6">
                ...
                {isDataLoading ? (
                  ...
                  <EmptyState icon={FileText} title="Aucun document" description="Vous n'avez pas encore de document disponible." />
                ) : (
                  <div className="space-y-3">
                    {(documents || []).map((doc) => (
                      <div key={doc?.id || Math.random()} className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl border border-gray-200 hover:border-[#d4af37]/50 transition-colors">
                        <div className="w-12 h-12 bg-[#d4af37]/20 rounded-lg flex items-center justify-center">
                          <FileText className="w-6 h-6 text-[#d4af37]" />
                        </div>
                        <div className="flex-1">
                          <p className="text-[#0a0f1e] font-medium">{doc?.name || "Document"}</p>
                          <p className="text-sm text-gray-600">
                            {doc?.type || "Fichier"} • {formatFileSize(doc?.size || 0)} • {new Date(doc?.created_at || new Date()).toLocaleDateString('fr-FR')}
                          </p>
                        </div>
                        {doc?.url && (
                          <a href={doc.url} target="_blank" rel="noopener noreferrer" className="p-2 text-gray-600 hover:text-[#d4af37] transition-colors">
                ...
                <div className="grid md:grid-cols-2 gap-6 mb-8">
                  <div className="space-y-6">
                    <div>
                      ...
                      <input type="number" value={loanAmount} onChange={(e) => setLoanAmount(Number(e.target.value))} className="w-full px-4 py-3 bg-white border border-gray-300 rounded-xl text-[#0a0f1e] focus:border-[#d4af37] focus:outline-none transition-colors" />
                    </div>
                    <div>
                      <label className="block text-sm text-gray-700 mb-2">Apport Initial (FCFA)</label>
                      <input type="number" value={downPayment} onChange={(e) => setDownPayment(Number(e.target.value))} className="w-full px-4 py-3 bg-white border border-gray-300 rounded-xl text-[#0a0f1e] focus:border-[#d4af37] focus:outline-none transition-colors" />
                    </div>
                    <div>
                      <label className="block text-sm text-gray-700 mb-2">Durée (années)</label>
                      <input type="number" value={loanTerm} onChange={(e) => setLoanTerm(Number(e.target.value))} className="w-full px-4 py-3 bg-white border border-gray-300 rounded-xl text-[#0a0f1e] focus:border-[#d4af37] focus:outline-none transition-colors" />
                    </div>
                    <div>
                      <label className="block text-sm text-gray-700 mb-2">Taux d'Intérêt (%)</label>
                      <input type="number" step="0.1" value={interestRate} onChange={(e) => setInterestRate(Number(e.target.value))} className="w-full px-4 py-3 bg-white border border-gray-300 rounded-xl text-[#0a0f1e] focus:border-[#d4af37] focus:outline-none transition-colors" />
                    </div>
                  </div>
