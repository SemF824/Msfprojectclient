...
import {
  ...
  Building2, MapPin, FileText, Mail, User, Loader2
} from "lucide-react";
...
export default function TransactionDetail() {
  ...
  useEffect(() => {
    ...
    const fetchTransactionDetails = async () => {
      ...
      try {
        ...
        console.error("Erreur de chargement transaction:", err);
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: `Transaction MSF Congo - ${transaction?.id}`,
        text: `Détails du versement pour ${transaction?.property_name}`,
        url: window.location.href,
      }).catch(console.error);
    } else {
      navigator.clipboard.writeText(window.location.href);
      toast.success("Lien de la transaction copié dans le presse-papiers.");
    }
  };

  const handleDownloadReceipt = () => {
    toast.success("Génération du reçu officiel en cours de téléchargement...");
  };

  const handleEmailReceipt = () => {
    toast.success(`Le reçu officiel est en cours d'envoi à l'adresse : ${user?.email}`);
  };
  ...
  if (isLoading) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-gray-500 font-medium">Chargement des détails de la transaction...</p>
      </div>
    );
  }
  ...
  if (error || !transaction) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4 text-center px-4">
        ...
        <Link to="/client/transactions" className="mt-4 px-6 py-2.5 bg-[#d4af37] text-[#0a0f1e] font-bold rounded-xl hover:bg-[#b8952e] transition-colors shadow-sm">
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto px-2 sm:px-0 text-[#0a0f1e]">
      {/* Header Actions */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <Link 
          to="/client/transactions" 
          className="inline-flex items-center gap-2 text-gray-500 hover:text-[#d4af37] transition-colors font-medium text-sm"
        >
        ...
        <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
          <button 
            onClick={handlePrint}
            className="flex-1 sm:flex-none p-2.5 text-gray-600 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors shadow-sm flex items-center justify-center" 
            title="Imprimer"
          >
          ...
          <button 
            ...
            className="flex-[2] sm:flex-none flex items-center justify-center gap-2 px-6 py-2.5 bg-[#d4af37] text-[#0a0f1e] font-bold rounded-xl hover:bg-[#b8952e] transition-colors shadow-sm text-sm"
          >
          ...
          >
            ...
            <div className="bg-gradient-to-br from-[#d4af37]/5 to-[#f4e3b2]/10 rounded-2xl p-5 sm:p-6 md:p-8 border border-[#d4af37]/20 relative overflow-hidden">
              <div className="relative z-10 w-full">
                ...
                <p className="text-2xl sm:text-4xl md:text-5xl font-black text-[#d4af37] mb-2 break-words break-all sm:break-normal leading-tight w-full tracking-tight">
            ...
            <div className="mt-6 pt-6 border-t border-gray-100">
               <p className="text-xs text-gray-400 font-mono break-all bg-gray-50 border border-gray-100 p-2 rounded-lg">ID Transaction : {transaction.id}</p>
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="mb-6">
              ...
              <p className="flex items-start sm:items-center gap-2 text-gray-600 text-xs sm:text-sm">
                ...
                <span className="break-all text-gray-400 font-mono">ID Objet : {transaction.property_id}</span>
              </p>
            </div>
        ...
        <div className="space-y-6 w-full min-w-0">
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <a href={`mailto:${user?.email}`} className="flex items-center gap-3 text-sm text-gray-600 hover:text-[#d4af37] break-all font-medium">
                  <Mail className="w-4 h-4 flex-shrink-0 text-gray-400" /> {user?.email}
                </a>
              </div>
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="space-y-3 w-full">
              <button 
                onClick={handleDownloadReceipt}
                className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-gray-50 hover:bg-gray-100 border border-gray-200 text-[#0a0f1e] font-semibold rounded-xl transition-colors text-sm"
              >
          ...
          <motion.div 
            ...
            className="bg-gradient-to-br from-[#0a0f1e] to-[#1a2540] rounded-3xl border border-[#d4af37]/20 p-5 sm:p-6 text-white shadow-lg"
          >
            ...
            <Link 
              to="/contact"
              className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-[#d4af37] hover:bg-[#b8952e] text-[#0a0f1e] font-bold rounded-xl transition-colors text-sm shadow-sm"
            >
